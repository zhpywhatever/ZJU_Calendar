...
class YunfengSpider(scrapy.Spider):
    ...
    def __init__(self):
        ...
        self.conn = sqlite3.connect('../yunfeng_notices.db')
            ...
            if date_text:
                ...
                if link:
                    ...
                    yield scrapy.Request(full_link, callback=self.parse_activity_page,
                                         meta={'title': title, 'date': date_text})
    ...
    def parse_activity_page(self, response):
        ...
        # 保存到 SQLite 数据库
        self.save_to_db(title, date, content, summary)
    ...
    def save_to_db(self, title, date, content, summary):
        # 将数据插入到 SQLite 数据库
        self.cursor.execute('''
            INSERT INTO notices (title, date, content, summary) VALUES (?, ?, ?, ?)
        ''', (title, date, content, summary))
        self.conn.commit()
