...
class BksySpider(scrapy.Spider):
    ...
    def setup_db(self):
        # Connect to SQLite database 'notices' and create 'bksy' table if it doesn't exist
        self.conn = sqlite3.connect('../../notices.db')
        self.cursor = self.conn.cursor()
        self.cursor.execute('''
            CREATE TABLE IF NOT EXISTS bksy (
    ...
    def save_to_db(self, data):
        self.cursor.execute('''
            INSERT INTO bksy (title, date, link, content) VALUES (?, ?, ?, ?)
