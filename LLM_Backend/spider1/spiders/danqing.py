...
class DQXYSpider(scrapy.Spider):
    name = 'dqxy'
    allowed_domains = ['dqxy.zju.edu.cn']
    start_urls = ['https://dqxy.zju.edu.cn/51432/list1.htm']
    counter = 1

    def __init__(self):
        self.stop_crawling = False
        self.base_url = 'https://dqxy.zju.edu.cn'
        self.today = datetime.now()
        self.three_months_ago = self.today - timedelta(days=90)
        self.setup_db()

    def setup_db(self):
        # Connect to SQLite database 'notices' and create 'danqing' table if it doesn't exist
        self.conn = sqlite3.connect('../../notices.db')
        self.cursor = self.conn.cursor()
        self.cursor.execute('''
            CREATE TABLE IF NOT EXISTS danqing (
    ...
    def save_to_db(self, data):
        self.cursor.execute('''
            INSERT INTO danqing (title, date, link, content) VALUES (?, ?, ?, ?)
